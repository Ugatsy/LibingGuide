...
@push('head')
    ...
    <style>
        ...
        .search-container { position: absolute; top: 20px; left: 50%; transform: translateX(-50%); z-index: 1000; width: 90%; max-width: 500px; }
        ...
        .spinner { display: inline-block; width: 18px; height: 18px; border: 3px solid rgba(255,255,255,0.3); border-radius: 50%; border-top-color: white; animation: spin 0.8s linear infinite; margin-right: 8px; vertical-align: middle; }
        ...
        .entrance-marker-label { background: #059669; color: white; padding: 2px 10px; border-radius: 9999px; font-size: 0.75rem; font-weight: 600; border: 2px solid white; box-shadow: 0 2px 6px rgba(0,0,0,0.3); white-space: nowrap; }
        @media (min-width: 1024px) {
            .find-wrap { top: 80px; }
        }
        @media (max-width: 640px) {
            .search-container { top: 16px; }
...
@section('content')
    <div class="find-wrap">
        ...
        <div id="searchContainer" class="search-container">
            ...
            <input type="text" id="searchBox" class="search-box" placeholder="Search for a grave by name or plot...">
    ...
    <script>
        ...
        async function loadCemeteryData() {
            try {
                // Load cemeteries and pick the first one with a polygon
                const cemRes = await fetch('{{ route('cemetery.list') }}');
                const cemeteries = await cemRes.json();

                if (cemeteries.length > 0) {
                    const activeCemetery = cemeteries.find(c => c.has_polygon) || cemeteries[0];
                    cemeteryId = activeCemetery.id;

                    const [polyRes, markersRes] = await Promise.all([
                        fetch('{{ route('cemetery.polygon.get') }}?cemetery_id=' + cemeteryId),
                    ...
                    if (markersData && markersData.features) {
                        renderAllMarkers(markersData.features);
                    }
                }
            } catch (e) {
        ...
        function renderAllMarkers(features) {
            features.forEach(f => {
                ...
                marker.bindPopup(`
                    ...
                    <div class="popup-meta">${props.plot_number || ''} ${props.section ? '— ' + props.section : ''}</div>
                `);

                marker.on('click', () => {
                    flyToBurial(coords[1], coords[0], props.full_name, props.plot_number || '', props.section || '',
        ...
        async function fetchEntranceNode() {
            if (!cemeteryId) return null;
            try {
                const res = await fetch('{{ route('cemetery.list') }}');
                const cemeteries = await res.json();
                const c = cemeteries.find(cem => cem.id === cemeteryId);
                return c || null;
            } catch (e) {
                return null;
            }
        ...
        function renderPath(data, endLat, endLng) {
            ...
            const startIcon = L.divIcon({
                className: '',
                html: '<div style="width:20px;height:20px;background:#ea580c;border:3px solid #fff;border-radius:50%;box-shadow:0 2px 8px rgba(0,0,0,0.4);"><div style="width:6px;height:6px;background:#fff;border-radius:50%;position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);"></div></div>',
                iconSize: [20, 20],
                iconAnchor: [10, 10],
            });
            ...
            const walkTime = Math.ceil(data.path.properties.distance * 12);
        ...
        async function findPath(endLat, endLng) {
            ...
            btn.disabled = true;
            btn.innerHTML = '<span class="spinner"></span> Finding path...';
            ...
            try {
                ...
                if (cemeteryId) params.append('cemetery_id', cemeteryId);

                const res = await fetch('{{ route('cemetery.find-path') }}?' + params.toString());
                ...
                map.flyTo([endLat, endLng], 19, { duration: 1.5 });
        ...
        function showDetailPanel(name, plot, section, dates, lat, lng) {
            ...
            content.innerHTML = `
                ...
                <button class="btn-directions" onclick="findPath(${lat}, ${lng})">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7"/></svg>Show Path
                </button>
                <div id="pathInfo" class="text-sm text-gray-500 mt-2"></div>
                <p class="text-xs text-gray-400 mt-2">Path starts from the cemetery entrance</p>
            `;
